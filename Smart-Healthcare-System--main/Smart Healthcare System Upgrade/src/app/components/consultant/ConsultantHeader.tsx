...
export default function ConsultantHeader() {
  ...
  const notifications = [
    { id: 1, text: 'Bệnh nhân mới cần tư vấn về thuốc', time: '3 phút trước', unread: true },
    { id: 2, text: 'Lịch tư vấn ngày mai đã được xác nhận', time: '45 phút trước', unread: true },
    { id: 3, text: 'Đánh giá từ bệnh nhân: 5 sao', time: '2 giờ trước', unread: false },
  ];

  const handleLogout = () => { navigate('/'); };

  const isActive = (path: string) =>
    path === '/consultant' ? location.pathname === '/consultant' : location.pathname.startsWith(path);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          <Link to="/consultant" className="flex items-center gap-2 hover:opacity-80 transition-opacity">
            <div className="bg-gradient-to-br from-teal-600 to-cyan-500 p-2 rounded-xl shadow-sm">
              <Heart className="size-5 text-white" fill="white" />
            </div>
            <div>
              <h1 className="font-bold text-lg text-gray-900 leading-tight">MediCare</h1>
              <p className="text-xs text-teal-600 font-medium -mt-0.5">Consultant Portal</p>
            </div>
          </Link>
            ...
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  {active && (
                    <motion.div
                      layoutId="consultant-nav-indicator"
                      className="absolute bottom-0 left-0 right-0 h-0.5 bg-gradient-to-r from-teal-600 to-cyan-500 rounded-full"
          ...
          <div className="flex items-center gap-3">
                    ...
                    >
                        ...
                        {notifications.map((notif) => (
                          ...
                          >
                            <div className="flex items-start gap-2">
                              {notif.unread && (
                                <span className="size-2 bg-teal-500 rounded-full mt-2 flex-shrink-0" />
                              )}
                      ...
                      <div className="px-4 py-3 bg-gray-50 border-t border-gray-100">
                        <button
                          ...
                          className="w-full text-sm text-teal-600 hover:text-blue-700 font-medium text-center"
                        >
            ...
            <SettingsModal isOpen={showSettings} onClose={() => setShowSettings(false)} role="consultant" />

            <div className="flex items-center gap-2 pl-2 border-l border-gray-200">
              <div className="size-9 bg-gradient-to-br from-teal-600 to-cyan-500 rounded-full flex items-center justify-center text-white font-bold text-sm shadow-sm">
                CN
              </div>
              <div className="hidden sm:block">
                <p className="text-sm font-semibold text-gray-900 leading-tight">Tư vấn viên</p>
                <p className="text-xs text-teal-600 font-medium -mt-0.5">Consultant</p>
              </div>
